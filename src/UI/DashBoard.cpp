...
DashBoard::DashBoard(int _radius, int _minValue, int _maxValue) : radius(_radius),minVal(_minValue),maxVal(_maxValue)
{
...
void DashBoard::drawScaleNum(QPainter* painter)
{
    ...
    for (int i = 0; i <= 10; i += 1)
    {
        ...
        int w = (int)fm.horizontalAdvance(value);
        int h = (int)fm.height();
        x = x - w / 2;
        y = y + h / 4;
...
void DashBoard::drawIndicator(QPainter* painter)
{
    ...
    double degRotate = startAngle + (float)currentValue/(maxVal-minVal) * 2 * M_PI * (360-startAngle*2)/360.0 * rad2Deg;
...
void DashBoard::drawText(QPainter* painter)
{
    ...
    painter->setPen(QPen(QColor(255, 255, 255)));
    QFontMetricsF fm = QFontMetricsF(painter->font());
    QString speed = QString::number(currentValue) + " mm/min";
