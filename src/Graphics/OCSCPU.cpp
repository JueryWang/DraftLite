...
namespace CNCSYS
{
	OCSCPU::OCSCPU(std::shared_ptr<SketchCPU> sketch) : sketch(sketch)
	{
	...
	void OCSCPU::SetCanvasSizae(int width, int height)
	{
		canvasWidth = width;
		canvasHeight = height;
	}

	void OCSCPU::ComputeScaleFitToCanvas()
	{
		...
		if (sketch && sketch->entities.size() > 0)
		{
			...
			float maxExpand = qMax(widthExpand, heightExpand);
	...
	void OCSCPU::UpdateTickers()
	{
		...
		while (beginX < endX)
		{
			...
			glm::vec2 lineStart = glm::vec2(2 * (beginX - canvasRange->min.x) / canvasRange->XRange() - 1.0f + 0.08f, -1.0f);
		...
		while (beginY < endY)
		{
			...
			glm::vec2 lineStart = glm::vec2(-1.0f, 2 * (beginY - canvasRange->min.y) / canvasRange->YRange() - 1.0f + 0.08f);
	...
	void OCSCPU::OnMouseScroll(float delta, const glm::vec2& mousePosition)
	{
	...
	void OCSCPU::OnMouseMove(const glm::vec2& offset)
	{
	...
	glm::vec3 OCSCPU::GetOCSPosWithPixelPos(const glm::vec2& pixelPos)
	{
